...
public class PrincipalOauth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        // 구글로그인 버튼 클릭 -> 구글 로그인 창 -> 로그인 완료 -> Access Token 요청
        // userRequest정보 -> loadUser 함수 호출 -> 구글로부터 넘어온 회원프로필 추출
        // -> 회원가입 -> OAuth2User -> Authentication

        String provider = userRequest.getClientRegistration().getClientId(); // google
        String providerId = oAuth2User.getAttribute("sub"); // 숫자
        String username = providerId + "_" + providerId; // google_숫자
